...
def job_outputs_complete(out_dir: Path) -> bool:
    return all((out_dir / name).exists() for name in REQUIRED_JOB_OUTPUTS)


def run_command(command: list[str], cwd: Path, skip_existing_dir: Path | None = None) -> None:
    if skip_existing_dir is not None:
        if job_outputs_complete(skip_existing_dir):
            print(f"[skip] {skip_existing_dir}")
            return
        existing = [name for name in REQUIRED_JOB_OUTPUTS if (skip_existing_dir / name).exists()]
        if existing:
            print(f"[rerun-incomplete] {skip_existing_dir} has only {', '.join(existing)}")
        ...
        for seed in args.seeds:
            for job in jobs:
                ...
                run_command(
                    ...
                    project_root,
                    out_dir if args.skip_existing else None,
                )
